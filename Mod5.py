...
class AnimalShelter(object):
    ...
    def read(self, search_parameter):
        ...
        # and then return search result if not null, and error message if null
        if search_parameter is not None:
            if search_parameter:
                search_result = self.database.animals.find(search_parameter)
                return search_result
        else:
            error_msg = "Nothing to search, because search parameter is empty."
            return error_msg
